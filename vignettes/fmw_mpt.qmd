...
```{r}
shiny::sliderInput("rho", "rho", min = -1, max = 1, value = 0, step = 0.01, post = "%")
shiny::plotOutput("correlated_normals")
```
```{r}
#| context: server 
output$correlated_normals <- shiny::renderPlot(plot_correlated_normals(input$rho))
```
